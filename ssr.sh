...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/gaojianrule/omg/master/shadowsocksr-manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/gaojianrule/omg/master/ssr_centos -O /etc/init.d/ssr; then
